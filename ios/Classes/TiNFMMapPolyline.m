...
@implementation TiNFMMapPolyline

@synthesize polyline, strokeColor, fillColor, points, polylineID, lineWidth, strokeAlpha, fillAlpha, coordinate = center,boundingRect;
...
//		self.boundingRect = MKMapRectMake(minLon,minLat,maxLon - minLon,maxLat-minLat); 
		...
		//initialize the native polyline object with the coordinats passed
		self.polyline = [MKPolyline polylineWithCoordinates:coords count:points_.count];
	}	
	return self;
}
...
-(void) dealloc
{
	...
	RELEASE_TO_NIL(polylineID);
	//TODO consider better ways of handling the deallocation of space for polyline, rather than having the mapkit object do it.
	//RELEASE_TO_NIL(polyline);  //Note:  This is NOT deallocated automatically because when you remove this <Overlay> from a mapkit map object, the mkpolyline will be deallocated automagically, and then attempts to deallocate this will fail with EXC_BAD_ACCESS
