...
@implementation NetfunctionalMapoverlayKMLDocumentProxy

#pragma mark titanium setter/getter and helper methods

-(void) setSourceFile:(id) kmlSourceFile {
    NSLog(@"[INFO] set source file called");
    UIImage* insetImageUI;
    NSURL* kmlURL;
    if ([kmlSourceFile isKindOfClass:[TiBlob class]]) {
        NSLog(@"tiblob passed");
...
//        TiBlob* imageBlob = (TiBlob*) tileDB;
        ...
        kmlURL = [NSURL URLWithString:[((TiBlob*) kmlSourceFile) nativePath ]];
    }
    else if ([kmlSourceFile isKindOfClass:[NSURL class]]) {
        NSLog(@"nsurl passed");
        kmlURL = kmlSourceFile;
    }
    else if([kmlSourceFile isKindOfClass:[NSString class]]) {
        NSLog(@"string passed");
        //        [NSFileManager defaultManager]
        
        TiFile* dbFileRef = [self getTiFile:kmlSourceFile];
        //        [dbFileRef ]
        
        NSString* tileDBFilePath = [dbFileRef path];
        
        NSLog(@"path passed for kml source file is %@",tileDBFilePath);
...
//
        kmlURL = [NSURL URLWithString:kmlSourceFile];
...
//
        ...
        NSLog(@"tiblob/string/nsurl not passed, instead found %@",[kmlSourceFile class]);
        return;
    }
    kmlSourceFileURLPath = [[kmlURL path] retain];
    _styles = [[NSMutableDictionary alloc] init];
    _placemarks = [[NSMutableArray alloc] init];
    _xmlParser = [[NSXMLParser alloc] initWithContentsOfURL:kmlURL];
...
#pragma mark internal method from original Apple kmlparser source code
...
//mmatan override initializer to set fill to true by default; this to deal with some of the weirdness of the satcom kml files, some of which explicitly include the fill flag, and others which do not.
...
- (void)applyToOverlayPathView:(MKOverlayPathView *)view
{
    ...
    if (fill) {
        //only give the view a fill color if the fill property has been set
